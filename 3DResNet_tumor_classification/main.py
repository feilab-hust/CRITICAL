...
### 设置数据集
    print('加载数据集')
    data_dir = r'D:\Data\Sunlab\Dataset_tumor\Train_Val_Dataset'  # 训练数据集所在的目录
    ...
    # train_images_path, train_images_label, val_images_path, val_images_label = read_split_data(data_dir, val_rate = 0.2, random_seed = 0)
    data_transform = {
            "train": transforms.Compose([Resize3D(size=(imgsize, imgsize, imgsize))]),
            "val": transforms.Compose([Resize3D(size=(imgsize, imgsize, imgsize))])}
    # 实例化数据集
